...
class Page:
    ...
    def on_web_socket(self, fn: Callable) -> None:
        ...
        Blocks until the engine has acknowledged the monitor install, and raises
        if it failed — the only place a sync caller can see it (#351).
        """
        # Must run via loop thread: on_web_socket() schedules the setup command
        # and needs a running event loop. Awaiting it here cannot deadlock — the
        # coroutine runs *on* the loop thread, which stays free to run the
        # receive loop that resolves it.
        async def _register() -> None:
