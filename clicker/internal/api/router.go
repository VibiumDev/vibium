...
func (r *Router) OnClientConnect(client ClientTransport) {
	...
	// transport starts reading client messages (server.handleWebSocket,
	// cmd/clicker/pipe.go), so finishing here is what guarantees no command is
	// served first. Backgrounded, a client whose first command started a
	// download could beat it, and the file landed in the browser's own download
	// directory where download.saveAs could not find it (#351).
