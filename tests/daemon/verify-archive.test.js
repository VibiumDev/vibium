...
test('Verify input reads Vibium and Playwright archives without a browser and writes only explicit reports', { timeout: 120000 }, async t => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'vibium-verify-archive-'));
  const env = { ...process.env, VIBIUM_SESSION: `verify-archives-${process.pid}`, VIBIUM_CONNECT_URL: '', VIBIUM_ENGINE: 'chrome', VIBIUM_ENGINE_PATH: '', VIBIUM_ENGINE_CHANNEL: '', VIBIUM_VERIFIER_PROVIDER: 'openai-compatible', VIBIUM_VERIFIER_MODEL: 'fixture', OPENAI_API_KEY: 'fixture-secret' };
  let calls = 0;
  let failure = false;
  ...
  try {
    ...
    await cli('verify', 'fixture generation: the order confirmation heading is visible');
    ...
    const playwright = path.resolve(__dirname, '../fixtures/verify/playwright-checkout.zip');
    for (const input of [vibium, playwright]) {
      ...
      for (const [flag, status] of [['-i', 'passed'], ['--input', 'failed'], ['--input', 'inconclusive']]) {
        ...
        const result = await cli('verify', claim, flag, input, '--report', report);
      ...
      const before = fs.readdirSync(dir).sort();
      await cli('verify', `${input === vibium ? 'vibium:' : 'playwright:'} inspect the confirmation heading`, '-i', input);
      assert.deepEqual(fs.readdirSync(dir).sort(), before, 'No artifact written by default');
      await reject(['verify', 'claim', '-i', input, '-o', path.join(dir, 'derived.zip')], /cannot be combined/);
      await reject(['verify', 'claim', '-i', input, '--report', input], /already exists/);
      assert.equal(hash(input), originalHash);
    }
    const invalid = path.join(dir, 'invalid.zip'); fs.writeFileSync(invalid, 'not a zip');
    await reject(['verify', 'claim', '-i', invalid], /archive|zip/);
    await reject(['verify', 'claim', '--record', vibium], /unknown flag/);
    await reject(['verify', 'claim', '--trace', playwright], /unknown flag/);
    failure = true;
    const report = path.join(dir, 'failed-report.json');
    await assert.rejects(cli('verify', 'claim', '-i', playwright, '--report', report), e => {
