...
async function lifecycle(t, { status = 'passed', keepOpen = false, existing = false, idleDaemon = false, mismatch = false, record = true } = {}) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'verify-lifecycle-'));
  let requests = 0;
  ...
  const env = { ...process.env, VIBIUM_SESSION: `vl-${process.pid}`,
    VIBIUM_ENGINE: process.env.VIBIUM_VERIFY_ENGINE || 'chrome', VIBIUM_ENGINE_PATH: '',
    VIBIUM_ENGINE_CHANNEL: process.env.VIBIUM_VERIFY_ENGINE === 'firefox' ? 'beta' : '',
    VIBIUM_CONNECT_URL: '', VIBIUM_VERIFIER_PROVIDER: 'openai-compatible',
    VIBIUM_VERIFIER_MODEL: 'fixture', VIBIUM_VERIFIER_REASONING_EFFORT: '',
    VIBIUM_VERIFIER_BASE_URL: `http://127.0.0.1:${provider.address().port}/v1`, OPENAI_API_KEY: '' };
  ...
  try {
    ...
    const args = ['verify', 'the fixture is visible', '--report', report];
    ...
    if (!mismatch && record) {
      // Evidence must be finalized before Verify returns and before cleanup.
      ...
      assert.ok(parent, 'missing Verify recording group');
      ...
      if (env.VIBIUM_ENGINE === 'firefox' && !existing) {
        assert.ok(videos.length > 0, 'Verify-owned Firefox recording lost its WebM track');
...
for (const status of ['passed', 'failed', 'inconclusive', 'error']) {
  test(`Verify closes its new browser after ${status} and saves evidence`, t => lifecycle(t, { status, idleDaemon: status === 'inconclusive' }));
  test(`Verify --keep-open retains its new browser after ${status}`, t => lifecycle(t, { status, keepOpen: true }));
}
for (const status of ['passed', 'error']) {
  test(`Verify preserves an existing browser and recording after ${status}`, t => lifecycle(t, { status, existing: true }));
}
test('Verify preserves the existing browser when launch flags conflict', t => lifecycle(t, { existing: true, mismatch: true }));

test('Verify also closes its new browser without --output', t => lifecycle(t, { record: false }));
