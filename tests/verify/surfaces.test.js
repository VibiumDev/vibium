...
test('Verify SDK and MCP surfaces share the native runtime', { timeout: 300000 }, async t => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'verify-surfaces-'));
  const errors = [];
  ...
  const env = { ...process.env, VIBIUM_BIN_PATH: path.join(root, 'clicker/bin/vibium'), VIBIUM_CONNECT_URL: '', VIBIUM_ENGINE: 'chrome', VIBIUM_ENGINE_PATH: '', VIBIUM_ENGINE_CHANNEL: '', VIBIUM_VERIFIER_PROVIDER: 'openai-compatible', VIBIUM_VERIFIER_MODEL: 'fixture', OPENAI_API_KEY: 'fixture-secret', VIBIUM_VERIFIER_BASE_URL: `${await listen(provider)}/v1`, VERIFY_URL: await listen(app), VERIFY_INPUT: path.join(root, 'tests/fixtures/verify/playwright-checkout.zip') };
  try {
    const runners = [
      ...
      ['Java', 'java', ['-cp', path.join(root, 'clients/java/build/libs/*') + path.delimiter + path.join(root, 'clients/java/build/dependencies/*'), path.join(__dirname, 'VerifySDK.java')], '0'],
    ...
    for (const [label, binary, args, sync] of runners) {
      for (const archived of ['0', '1']) {
        await t.test(`${label} ${archived === '1' ? 'archive without browser' : 'live pinned page'}`, async () => {
          ...
          const localEnv = { ...env, VERIFY_ENGINE: label.includes('Firefox') ? 'firefox' : 'chrome', VERIFY_SYNC: sync, VERIFY_ARCHIVE_ONLY: archived, VERIFY_OUTPUT: output };
