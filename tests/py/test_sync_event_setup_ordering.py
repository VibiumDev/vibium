...
``on_web_socket()`` must not return until the engine has acknowledged the install —
and must raise if the install was rejected, the only place a sync caller can see it.
See ``helpers/fake_engine.py``.
"""
...
def test_rejected_install_raises_from_on_web_socket(monkeypatch):
    # Set before launching: the stand-in reads it at startup.
    monkeypatch.setenv("VIBIUM_BIN_PATH", FAKE_ENGINE)
